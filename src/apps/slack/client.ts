...
export async function postToSlackChannel(
  ...
  console.log(
    "[Slack Client] Posting to channel:",
...
export async function isChannelAllowed(
  ...
  if (error) {
    console.log(
      "[Slack Client] Error fetching allowed channels:",
      error.message,
    );
    return false;
  }
  ...
  if (
    !integration?.allowed_channel_ids ||
...
export async function addReaction(
  ...
  console.log("[Slack Client] addReaction called:", {
    orgId,
    channelId,
    timestamp,
    emoji,
  ...
  const data = await res.json();
  console.log(
    "[Slack Client] reactions.add response:",
    JSON.stringify(data, null, 2),
  );
  return data.ok === true;
}
...
export async function lookupOrgBySlackTeam(
  ...
  if (!org) {
    console.log(
      "[Slack Client] Integration found but no linked org for team_id:",
      teamId,
    );
    return null;
  }
