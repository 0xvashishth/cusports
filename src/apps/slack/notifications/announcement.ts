...
export async function postAnnouncementToSlack(
  ...
  console.log("[Slack Notifications] postAnnouncementToSlack called:", {
    orgId,
    channelId,
    title,
  });
  ...
  if (error) {
    console.log(
      "[Slack Notifications] Error fetching integration:",
      error.message,
    );
    return false;
  }
  ...
  if (!integration?.slack_bot_token_encrypted) {
    console.log(
      "[Slack Notifications] No bot token configured, skipping announcement",
    );
    return false;
  }
  ...
  if (!channelAllowed) {
    console.log(
      "[Slack Notifications] Channel not in allowlist, skipping announcement",
    );
    return false;
  }
