...
export async function handlePlayerReport(
  ...
  if (!reporter) {
    return {
      success: false,
      replyMessage:
        "Your Slack email is not linked to any player account in this organization. Please contact a manager.",
    };
  }
  ...
  if (reporter.id === opponent.id) {
    return {
      success: false,
      replyMessage: "You cannot report a match against yourself.",
    };
  }
  ...
  if (!match) {
    return {
      success: false,
      replyMessage: `No scheduled match found between you and ${opponent.fullName}. Check your upcoming fixtures.`,
    };
  }
  ...
  console.log("[Slack Report] Advancing match:", {
  ...
  console.log(
    "[Slack Report] Match recorded successfully, reacting with checkmark",
  );
  return {
    success: true,
    reaction: "white_check_mark",
    replyMessage: `*Result:* ${winnerName} vs ${loserName} (${scoreStr})${matchLabel ? `\n${matchLabel}` : ""}`,
  };
}
