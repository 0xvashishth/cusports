...
export async function handleManagerReport(
  ...
  console.log("[Slack ManagerReport] handleManagerReport:", { orgId, reporterName, opponentName, games, channelId })

  const reporter = await findPlayerByName(orgId, reporterName)
  if (!reporter) {
    return { success: false, replyMessage: `Player "${reporterName}" not found. Make sure the full name matches exactly.` }
  }
  ...
  if (reporter.id === opponent.id) {
    return { success: false, replyMessage: "Cannot report a match between the same player." }
  }
  ...
  if (!match) {
    return { success: false, replyMessage: `No scheduled match found between ${reporter.fullName} and ${opponent.fullName}.` }
  }
  ...
  return {
    ...
    replyMessage: `*Result (manager):* ${winnerName} defeated ${loserName} (${scoreStr})${matchLabel ? `\n${matchLabel}` : ""}`,
  }
}
...
export async function handleManagerWalkover(
  ...
  if (!winner) {
    return { success: false, replyMessage: `Player "${winnerName}" not found.` }
  }
  ...
  if (!match) {
    return { success: false, replyMessage: `No scheduled match found between ${winner.fullName} and ${loser.fullName}.` }
  }

  const result = await walkoverMatch(match.matchId, winner.id)

  if (!result.success) {
    return { success: false, replyMessage: `Failed to record walkover: ${result.error}` }
  }
