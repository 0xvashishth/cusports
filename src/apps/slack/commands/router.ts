...
export function parseCommand(text: string): ParsedCommand {
  ...
  if (reportMatchRaw) {
    ...
    if (games.length === 0) {
      console.log(
        "[Slack Router] Parsed report (raw mention) but no valid games",
      );
      return { type: "unknown" };
    }
    console.log("[Slack Router] Parsed report match (raw mention):", {
      opponentSlackUserId,
      games,
    });
  ...
  if (reportMatchCleaned) {
    ...
    console.log("[Slack Router] Parsed report match (name):", {
      opponentName,
      games,
    });
  ...
  if (walkoverRaw) {
    console.log("[Slack Router] Parsed walkover (raw mention):", {
      opponentSlackUserId: walkoverRaw[1],
    });
    return { type: "walkover", opponentSlackUserId: walkoverRaw[1] };
  }

  const walkoverMatch = cleaned.match(/report\s+walkover\s+(?:vs\s+)?@?(\S+)/i);
  if (walkoverMatch) {
  ...
  if (managerReportRaw) {
    ...
    if (games.length === 0) {
      console.log(
        "[Slack Router] Parsed manager report (raw mention) but no valid games",
      );
      return { type: "unknown" };
    }
    console.log("[Slack Router] Parsed manager report (raw mention):", {
      playerASlackUserId,
      playerBSlackUserId,
      games,
    });
  ...
  if (managerReportCleaned) {
    ...
    if (games.length === 0) {
      console.log(
        "[Slack Router] Parsed manager report (name) but no valid games",
      );
      return { type: "unknown" };
    }
    console.log("[Slack Router] Parsed manager report (name):", {
      playerA,
      playerB,
      games,
    });
...
export async function routeCommand(
  ...
  console.log("[Slack Router] routeCommand called:", {
    orgId,
    orgSlug,
    slackUserId,
    channelId,
  ...
  if (!botToken) {
    ...
    return {
      success: false,
      replyMessage: "Bot token not configured for this organization.",
    };
  }
  ...
  switch (parsed.type) {
    case "report": {
      const reporter = await findPlayerBySlackUserId(
        orgId,
        slackUserId,
        botToken,
      );
      ...
      if (!reporter) {
        return {
          success: false,
          replyMessage:
            "Your Slack email is not linked to any player account in this organization. Please make sure your Slack profile has your email set and it matches your player account.",
        };
      }
      ...
      if (parsed.opponentSlackUserId) {
        ...
        console.log(
          "[Slack Router] Opponent lookup by Slack user ID:",
          opponent,
        );
        if (!opponent) {
          return {
      ...
      if (!opponent) {
        return {
          success: false,
          replyMessage:
            "Could not identify the opponent. Please mention them with @username.",
        };
      }
    ...
    case "manager_report": {
      ...
      console.log(
        "[Slack Router] Manager report command - reporter:",
        reporter,
      );
      if (!reporter) {
        return {
          success: false,
          replyMessage: "Your Slack email is not linked to any player account.",
        };
      }
      ...
      if (!managerCheck) {
        return {
          success: false,
          replyMessage:
            "Only managers can use the 'report result @A vs @B' command.",
        };
      }
      ...
      if (!playerAName || !playerBName) {
        return {
          success: false,
          replyMessage:
            "Could not identify both players. Please mention them with @username.",
        };
      }
    ...
    case "walkover": {
      ...
      if (!managerCheck) {
        return {
          success: false,
          replyMessage: "Only managers can record walkovers.",
        };
      }
      return {
        success: false,
        replyMessage:
          "Walkover reporting via Slack is not yet implemented. Please use the dashboard.",
      };
    }
    ...
    case "help":
      return {
        success: true,
        replyMessage: [
          `*Cusports Table Tennis Bot Commands*`,
          "",
